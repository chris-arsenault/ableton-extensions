...
- **SDK is pinned (no longer guessed):** the Live 12.4.5 beta bundle is vendored (`vendor/@ableton-extensions/{sdk,cli}`) and `send-to-sulion` is wired against the real `@ableton-extensions/sdk` 1.0.0-beta.0 — real `activate`/`initialize`, `registerCommand`/`registerContextMenuAction`, `getObjectFromHandle`, `MidiClip.notes`, `withinProgressDialog`, and `environment.storageDirectory` (filesystem is not sandboxed). See [docs/extensions-sdk.md](docs/extensions-sdk.md) for the verified API. What remains is running it inside Live (M3).
